...
/// Elevation tiers used when requesting a shadow for a liquid-glass surface.
/// Each tier gets a slightly taller + softer shadow; dark mode always
/// returns an empty list because the tinted surfaces already carry enough
/// visual depth on dark backgrounds.
enum LiquidGlassElevation { surface, elevated, modal }

/// Returns a layered, subtle drop shadow for light mode — a tight "contact"
/// shadow plus a softer ambient shadow. Returns an empty list in dark mode
/// so we don't waste GPU cycles painting shadows that wouldn't be visible
/// anyway.
List<BoxShadow> liquidGlassShadows(
  BuildContext context, {
  LiquidGlassElevation level = LiquidGlassElevation.surface,
}) {
  if (Theme.of(context).brightness == Brightness.dark) return const [];
  switch (level) {
    case LiquidGlassElevation.surface:
      return const [
        // Very tight contact shadow defines the card edge against the bg.
        BoxShadow(
          color: Color(0x10000000),
          blurRadius: 2,
          offset: Offset(0, 1),
        ),
        // Softer ambient shadow gives depth without feeling heavy.
        BoxShadow(
    ...
    case LiquidGlassElevation.elevated:
      return const [
        BoxShadow(
    ...
    case LiquidGlassElevation.modal:
      return const [
        BoxShadow(
          color: Color(0x15000000),
          blurRadius: 4,
